...
def update_zod_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    METAINFO = {
        'classes': ['Vehicle', 'VulnerableVehicle', 'Pedestrian', 'Animal', 'PoleObject', 'TrafficBeacon', 
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    if dataset.lower() == 'kitti':
        ...
        update_zod_infos(pkl_path=pkl_path, out_dir=out_dir)
    else:
